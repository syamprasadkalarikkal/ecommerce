...
export default function ProductModal({ productId, onClose, onReviewSubmit }) {
      ...
      <div className="bg-white max-w-xl w-full p-6 rounded-lg relative max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex flex-col md:flex-row gap-4">
          <div className="md:w-1/2">
            <Image
              src={product.image?.startsWith('file://') ? `/images/${product.image.split('/').pop().replace(/_\d+\.png$/, '.png').replace('featured_lady_model_', 'model').replace('ladies_fashion_hero', 'hero')}` : product.image}
              alt={product.name}
              width={400}
              height={400}
              unoptimized
          ...
          <div className="md:w-1/2">
            ...
            <div className="flex items-center mb-2">
              {[...Array(5)].map((_, idx) => (
                <FaStar
                  ...
                  size={20}
                  color={
                    idx < Math.round(productRating.rate) ? "#FFC107" : "#E4E5E9"
                  }
                />
              ))}
            ...
            <button
              ...
              disabled={isInCart}
              className={`px-4 py-2 rounded ${isInCart
